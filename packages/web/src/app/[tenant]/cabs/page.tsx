...
export default function CABsPage() {
  ...
  // Fetch CABs from tRPC with default pagination
  const { data, isLoading, error } = trpc.cabs.list.useQuery({
    limit: 20,
    offset: 0,
    orderBy: 'createdAt',
    orderDirection: 'desc',
    ...
    return (
      <div className="p-8">
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <h3 className="text-xl font-semibold mb-2">Failed to load CABs</h3>
          <p className="text-muted-foreground text-center max-w-md mb-6">
            {error.message || 'An error occurred while loading your advisory boards'}
          </p>
        </div>
      </div>
    );
